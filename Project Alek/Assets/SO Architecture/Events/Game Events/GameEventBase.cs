...
namespace ScriptableObjectArchitecture
{
    ...
    public abstract class GameEventBase<T,TU> : GameEventBase, IGameEvent<T,TU>, IStackTraceObject
    {
        private readonly List<IGameEventListener<T,TU>> _typedListeners = new List<IGameEventListener<T,TU>>();
        private readonly List<System.Action<T,TU>> _typedActions = new List<System.Action<T,TU>>();

        [SerializeField]
        protected T debugValue1 = default(T);
        [SerializeField]
        protected TU debugValue2 = default(TU);

        public void Raise(T value1, TU value2)
        {
            AddStackTrace(value1);
            ...
            for (int i = _typedActions.Count - 1; i >= 0; i--)
                _typedActions[i](value1,value2);

            for (int i = _actions.Count - 1; i >= 0; i--)
                _actions[i]();
        }
        public void AddListener(IGameEventListener<T,TU> listener)
        {
            if (!_typedListeners.Contains(listener))
                _typedListeners.Add(listener);
        }
        public void RemoveListener(IGameEventListener<T,TU> listener)
        {
            if (_typedListeners.Contains(listener))
                _typedListeners.Remove(listener);
        }
        public void AddListener(System.Action<T,TU> action)
        {
            if (!_typedActions.Contains(action))
                _typedActions.Add(action);
        }
        public void RemoveListener(System.Action<T,TU> action)
        {
            if (_typedActions.Contains(action))
                _typedActions.Remove(action);
        }
