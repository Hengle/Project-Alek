...
namespace BattleSystem
{
    ...
    public class BattleOptionsPanel : ScriptableObject
    {
        ...
        public void GetCommandInformation(string parameters)
        {
            ...
            var commandActionName = splittedParams[0];
            var commandActionOptionString = splittedParams[1];
            var commandTargetOptionsString = splittedParams[2];
            var commandCostString = splittedParams[3];
            ...
            var check = character.unit.currentAP - commandCost < 0;
            if (check) {Debug.Log("Not enough AP!"); return;}
